...
import { AthleteCard, Avatar, Button, CoachCard, EmptyState, OpportunityCard, PostCard } from "./components";
import { auth, db, isFirebaseConfigured } from "./firebase";
import Login from "./login";
import ProfileSetup from "./ProfileSetup";
import AthleteAnalyzer from "./AthleteAnalyzer";

const navItems = [["Home", "âŒ‚"], ["Discover", "âŒ•"], ["Reels", "â–·"], ["Coaches", "â™™"], ["Opportunities", "â—Ž"], ["Profile", "â—‰"]];
...
function App() {
  ...
  if (loading) return <div className="auth-loading">Loading SportsConnectâ€¦</div>;
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" onClick={() => navigate("Home")}><span className="brand-mark">S</span><span>Sports<span>Connect</span></span></button>
      <label className="global-search"><span>âŒ•</span><input placeholder="Search athletes, sports, opportunities" /></label>
      <div className="top-actions"><button className="notification" aria-label="Notifications">â™¢<i /></button><button className="avatar-button" onClick={() => navigate("Profile")}><Avatar person={me} size="sm" /></button><button className="menu-button" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Open navigation">â˜°</button></div>
    </header>
    <aside className={`side-nav ${mobileOpen ? "mobile-open" : ""}`}>{navItems.map(([name, icon]) => <button key={name} className={page === name ? "nav-active" : ""} onClick={() => navigate(name)}><span>{icon}</span>{name}</button>)}<div className="nav-foot"><p>YOUR SPORTING JOURNEY</p><strong>Built to be seen.</strong>{isFirebaseConfigured && <button className="logout-button" onClick={logout}>Log out</button>}</div></aside>
...
function Home({ onNavigate, me }) { const [postText, setPostText] = useState(""); return <>
  <section className="hero"><div><p className="eyebrow">WELCOME BACK, {me.name.split(" ")[0].toUpperCase()}</p><h1>Your next <em>breakthrough</em><br />starts here.</h1><p>Share your journey, find your people and uncover the opportunities that move you forward.</p><Button onClick={() => document.getElementById("create-post")?.focus()}>Share an update <span>â†’</span></Button></div><div className="hero-stat"><strong>12</strong><span>days to your<br />next milestone</span><div className="progress"><i /></div><small>State qualifier Â· 28 August</small></div></section>
  <div className="content-grid"><section><div className="section-title"><div><p className="eyebrow">YOUR FEED</p><h2>From the community</h2></div><button className="text-button">Following â–¾</button></div>
    <div className="create-post"><Avatar person={me}/><input id="create-post" value={postText} onChange={(e) => setPostText(e.target.value)} placeholder="Share a training update, achievement or goalâ€¦"/><Button variant="soft" disabled={!postText}>Post</Button></div>{posts.map((post) => <PostCard key={post.id} post={post} />)}</section>
    <aside className="right-rail"><div className="rail-card"><div className="section-title compact"><h3>Trending sports</h3><button className="text-button" onClick={() => onNavigate("Discover")}>Explore</button></div>{[["Athletics", "12.8k athletes", "â†—"], ["Badminton", "9.4k athletes", "â—‰"], ["Football", "7.9k athletes", "â—†"]].map(([name, count, icon]) => <div className="trend" key={name}><b>{icon}</b><div><strong>{name}</strong><p>{count}</p></div><span>â†—</span></div>)}</div>
...
function FilterBar({ query, setQuery, sport, setSport, location, setLocation, placeholder }) { return <div className="filter-bar"><label className="search-field">âŒ•<input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={placeholder}/></label><select value={sport} onChange={(e) => setSport(e.target.value)}>{sports.map((s) => <option key={s}>{s}</option>)}</select>{location !== undefined && <select value={location} onChange={(e) => setLocation(e.target.value)}>{locations.map((l) => <option key={l}>{l}</option>)}</select>}</div> }
...
function Reels() { const [liked, setLiked] = useState({}); return <PageHeader eyebrow="WATCH Â· LEARN Â· GET INSPIRED" title="Sports in motion" description="A quick look at the effort behind every performance."><div className="reels-grid">{posts.map((post, i) => <article className={`reel reel-${i}`} key={post.id}><div className="reel-gradient"/><div className="reel-play">â–·</div><div className="reel-copy"><div className="reel-person"><Avatar person={post.athlete} size="sm"/><strong>{post.athlete.name}</strong><Button variant="outline">Follow</Button></div><p>{post.text}</p><span>#{post.athlete.sport.toLowerCase()} #sportsconnect</span></div><div className="reel-actions"><button className={liked[post.id] ? "liked" : ""} onClick={() => setLiked({...liked, [post.id]: !liked[post.id]})}>{liked[post.id] ? "â™¥" : "â™¡"}<small>{post.likes + (liked[post.id] ? 1 : 0)}</small></button><button>â—Œ<small>{post.comments}</small></button><button>â†—<small>Share</small></button></div></article>)}</div></PageHeader> }
...
function Profile({ profile, isOwnProfile, onBack }) { const achievements = profile.achievements?.length ? profile.achievements : [["â˜…", "State 400m finalist", "2026"], ["â—ˆ", "Inter-college gold", "2025"], ["âœ¦", "Personal best: 48.92s", "400m"]]; return <>{!isOwnProfile && <button className="text-button profile-back-button" onClick={onBack}>Back to discover</button>}<section className="profile-hero"><div className="cover-pattern"/><div className="profile-main"><Avatar person={profile} size="xl"/><div><p className="eyebrow">ATHLETE PROFILE</p><h1>{profile.name}</h1><p className="muted">@{profile.username} Â· {profile.sport} Â· {profile.location}</p><p className="profile-bio">{profile.bio || "Building a sporting journey worth sharing."}</p></div>{isOwnProfile ? <Button variant="outline">Edit profile</Button> : <Button variant="outline">Follow</Button>}</div><div className="profile-stats">{[["1.8k", "Followers"], ["326", "Following"], [String(profile.achievements?.length || 14), "Achievements"], ["28", "Posts"]].map(([n, l]) => <div key={l}><strong>{n}</strong><span>{l}</span></div>)}</div></section>{isOwnProfile && <AthleteAnalyzer profile={profile}/>}<div className="profile-layout"><section><SectionTitle label="SPORTING HIGHLIGHTS" title="Achievements & medals"/><div className="highlights">{achievements.map((item, index) => { const [i, title, year] = Array.isArray(item) ? item : ["âœ¦", item.title, item.year]; return <article key={`${title}-${index}`}><b>{i}</b><div><strong>{title}</strong><p>{year}</p></div></article>; })}</div><SectionTitle label="RECENT ACTIVITY" title="Posts & reels"/><div className="profile-posts">{posts.map((p) => <PostCard key={p.id} post={p}/>)}</div></section><aside><div className="rail-card certificates"><h3>Certificates</h3>{["State Athletics Association", "Sports Science Foundation"].map((c) => <div key={c}><b>â–§</b><span><strong>{c}</strong><small>Verified credential</small></span><button>View</button></div>)}</div></aside></div></> }
